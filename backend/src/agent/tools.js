...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description: 'Propone pausar o crear conjuntos/campañas — incluye probar públicos/intereses nuevos, campañas de testeo, o duplicar una estructura que funciona con otro segmento. QUEDA PENDIENTE de aprobación humana (salvo que te lo pidan en el chat). Para crear un conjunto de prueba COMPLETO (público + presupuesto + pieza) en una sola aprobación: usá action=create_adset, payload con el targeting (con los interest_id que confirmaste con search_interest) y demás campos de Graph API EXCEPTO el presupuesto y el promoted_object (si optimization_goal=OFFSITE_CONVERSIONS, el pixel de la cuenta se agrega solo — no lo inventes ni lo pidas), daily_budget_ars con el presupuesto diario en PESOS (se convierte solo a centavos), y creative_id con el id de un creativo de unusedCreatives para que además cree el anuncio ya armado en ese conjunto nuevo. Graph API exige targeting.targeting_automation.advantage_audience (0 o 1) — si no lo poné explícito se usa 0 (Meta NO expande más allá del targeting que armaste); poné 1 solo si a propósito querés que Meta amplíe la segmentación buscando mejor performance. No hace falta esperar evidencia de que algo "no funciona" para proponer un test — proponelo si ves una oportunidad razonable (audiencia sin explorar, segmento del funnel desatendido, etc.), dejando claro en reason que es exploratorio.',
    input_schema: {
      type: 'object',
      properties: {
